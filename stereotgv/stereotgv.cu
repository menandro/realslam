...
int StereoTgv::copyMaskToDevice(cv::Mat mask) {
	...
	//std::cout << fisheyeMaskPad.size() << std::endl;
	for (int level = 1; level < nLevels; level++) {
...
int StereoTgv::solveStereoForwardCensusMasked() {
	// Warp i1 using vector fields
	//WarpImageMasked(pI1[0], pFisheyeMask[0], width, height, stride, d_cv, d_i1calibrated);
	WarpImage(pI1[0], width, height, stride, d_cv, d_i1calibrated);
	Swap(pI1[0], d_i1calibrated);
	...
	for (int level = nLevels - 1; level >= 0; level--) {
		...
		for (int warpIter = 0; warpIter < nWarpIters; warpIter++) {
			...
			WarpImage(pI1[level], pW[level], pH[level], pS[level], d_warpUV, d_i1warp);
			if (level == 2) {
				DEBUGIMAGE("in0", pI0[level], pH[level], pS[level], false, false);
				DEBUGIMAGE("in1", pI1[level], pH[level], pS[level], false, false);
				DEBUGIMAGE("iwarp", d_i1warp, pH[level], pS[level], false, false);
			}
			

			ComputeCensusFisheye(pI0[level], d_i1warp, censusThreshold, 
				pW[level], pH[level], pS[level], d_Iz);
			if (level == 2) {
				DEBUGIMAGE("Iz", d_Iz, pH[level], pS[level], false, false);
			}
			ComputeCensusDerivativesFisheye(d_Iz, pTvForward[level],
				pW[level], pH[level], pS[level], d_Iu);
