...
__global__ void TgvThresholdingL1Kernel(float2* Tp, float* u_, float2* v_, float2* p, float4* q,
...
void StereoTgv::ThresholdingL1(float2* Tp, float* u_, float2* v_, float2* p, float4* q,
	float* a, float* b, float* c,
	float tau, float* eta_u, float* eta_v1, float* eta_v2,
	float alpha0, float alpha1, float mu,
	int w, int h, int s,
	...
	TgvThresholdingL1Kernel << < blocks, threads >> > (Tp, u_, v_, p, q,
		a, b, c,
...
void StereoTgv::SolveTp(float* a, float* b, float* c, float2* p,
	...
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
	SolveTpKernel << < blocks, threads >> > (a, b, c, p, Tp, w, h, s);
}