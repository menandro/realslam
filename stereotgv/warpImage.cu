...
void StereoTgv::FindWarpingVectorEquidistant(const float2 *warpUV, const float2 *tv,
	int w, int h, int s, float2 *tv2)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
	...
	TgvFindWarpingVectorEquidistantFloat2Kernel << <blocks, threads >> > (warpUV, w, h, s, tv2);
}
