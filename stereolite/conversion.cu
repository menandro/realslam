...
// Conversion using Kannala-Brandt Model
__global__ void LiteConvertKBKernel(float2 *disparity,
...
void StereoLite::ConvertKB(float2 *disparity, float focalx, float focaly, float cx, float cy,
	...
	float3 *X, float* depth, int w, int h, int s)
{
	...
	LiteConvertKBKernel << <blocks, threads >> > (disparity,
		focalx, focaly, cx, cy,
		d1, d2, d3, d4,
		t1, t2, t3,
		X, depth,
