...
__global__ void LiteComputeDerivativesKernel(int width, int height, int stride,
...
///CUDA CALL FUNCTIONS ***********************************************************
void StereoLite::ComputeDerivatives(float *I0, float *I1,
	int w, int h, int s,
	float *Ix, float *Iy, float *Iz)
{
	...
	LiteComputeDerivativesKernel << < blocks, threads >> > (w, h, s, Ix, Iy, Iz);
}
...
//****************************************
__global__
void LiteComputeDerivativesFisheyeKernel(float2 * vector, int width, int height, int stride,
	float *Iw, float *Iz)
{
	const int ix = threadIdx.x + blockIdx.x * blockDim.x;
	const int iy = threadIdx.y + blockIdx.y * blockDim.y;
...
///CUDA CALL FUNCTIONS ***********************************************************
void StereoLite::ComputeDerivativesFisheye(float *I0, float *I1, float2 *vector,
	int w, int h, int s, float *Iw, float *Iz)
{
	...
	LiteComputeDerivativesFisheyeKernel << < blocks, threads >> > (vector, w, h, s, Iw, Iz);
}
