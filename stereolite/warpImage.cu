...
__global__ void LiteWarpingKernel(int width, int height, int stride,
...
void StereoLite::WarpImage(const float *src, int w, int h, int s,
	const float2 *warpUV, float *out)
{
	...
	LiteWarpingKernel << <blocks, threads >> > (w, h, s, warpUV, out);
}
...
__global__ void LiteFindWarpingVectorKernel(const float2 *warpUV,
...
void StereoLite::FindWarpingVector(const float2 *warpUV, const float *tvx, const float *tvy,
	int w, int h, int s, float2 *tv2)
{
	...
	LiteFindWarpingVectorKernel << <blocks, threads >> > (warpUV, w, h, s, tv2);
}
...
__global__ void LiteFindWarpingVectorFloat2Kernel(const float2 *warpUV,
...
void StereoLite::FindWarpingVector(const float2 *warpUV, const float2 *tv,
	int w, int h, int s, float2 *tv2)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
	...
	LiteFindWarpingVectorFloat2Kernel << <blocks, threads >> > (warpUV, w, h, s, tv2);
}
...
__global__ void LiteComputeOpticalFlowVectorKernel(const float *u, const float2 *tv2,
...
void StereoLite::ComputeOpticalFlowVector(const float *u, const float2 *tv2,
	int w, int h, int s, float2 *warpUV)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));

	LiteComputeOpticalFlowVectorKernel << <blocks, threads >> > (u, tv2, w, h, s, warpUV);
}
