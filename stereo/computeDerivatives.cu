...
//****************************************
__global__ 
void ComputeDerivativesFisheyeKernel(float * vectorX, float *vectorY, int width, int height, int stride,
	float *Iw, float *Iz)
{
	const int ix = threadIdx.x + blockIdx.x * blockDim.x;
	const int iy = threadIdx.y + blockIdx.y * blockDim.y;
	...
	float vx = vectorX[pos];
	float vy = vectorY[pos];
	...
	//float dx = vx / (float)width;
	//float dy = vy / (float)height;
...
///CUDA CALL FUNCTIONS ***********************************************************
void Stereo::ComputeDerivativesFisheye(float *I0, float *I1, float *vectorX, float *vectorY,
	int w, int h, int s,
	float *Iw, float *Iz)
{
	dim3 threads(BlockWidth, BlockHeight);
	...
	ComputeDerivativesFisheyeKernel << < blocks, threads >> > (vectorX, vectorY, w, h, s, Iw, Iz);
}
