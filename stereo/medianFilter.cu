...
__global__ 
void MedianFilterKernel5 (float* u, float* v, 
...
void Stereo::MedianFilter(float *inputu, float *inputv, 
	int w, int h, int s,
	float *outputu, float*outputv,
	int kernelsize)
{
...
void Stereo::MedianFilter3D(float *X, float *Y, float *Z,
	int w, int h, int s,
	float *X1, float *Y1, float *Z1,
	int kernelsize)
{
...
//*************************************
__global__
void MedianFilterDisparityKernel5(float* u,
	int width, int height, int stride,
	float *outputu)
{
	const int ix = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void MedianFilterDisparityKernel3(float* u,
	int width, int height, int stride,
	float *outputu)
{
	const int ix = threadIdx.x + blockIdx.x * blockDim.x;
...
void Stereo::MedianFilterDisparity(float *inputu,
	int w, int h, int s,
	float *outputu,
	int kernelsize)
{
	...
	if (kernelsize == 3) {
		MedianFilterDisparityKernel3 << < blocks, threads >> > (inputu,
			w, h, s, outputu);
	}
	else if (kernelsize == 5) {
		MedianFilterDisparityKernel5 << < blocks, threads >> > (inputu,
			w, h, s, outputu);
	}
}