...
void Stereo::Cv8uToGray(uchar * d_iCv8u, float *d_iGray, int w, int h, int s)
{
...
__global__
void Cv8uc3ToGrayKernel(uchar3 *d_iRgb, float *d_iGray, int width, int height, int stride)
{
	int r = blockIdx.y * blockDim.y + threadIdx.y;        // current row 
	int c = blockIdx.x * blockDim.x + threadIdx.x;        // current column 
...
void Stereo::Cv8uc3ToGray(uchar3 * d_iRgb, float *d_iGray, int w, int h, int s)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));

	Cv8uc3ToGrayKernel << < blocks, threads >> > (d_iRgb, d_iGray, w, h, s);
}