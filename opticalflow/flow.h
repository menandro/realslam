...
public:
	...
	// Output Optical Flow
	float2 *d_u, *d_us, *d_umed, *d_umeds;
	...
	void ThresholdingL1Masked(float* mask, float2* u, float2* u_med, float* Ix, float* Iy, float* Iz,
	...
	void MedianFilter(float2 *inputu, int w, int h, int s, float2 *outputu, int kernelsize);
