...
int test_savePointCloudAsCSV() {
	...
	for (int k = startFrame; k <= endFrame; k++) {
		...
		std::vector<cv::Mat> Xsplit;
		cv::split(Xraw, Xsplit);
		for (int ff = 0; ff < 3; ff++) {
			std::ofstream pcloudFile;
			std::string filename = mainfolder + "/csv/pc" + appender + std::to_string(k)
				+ "_" + std::to_string(ff) + ".csv";
			pcloudFile.open(filename.c_str());
			pcloudFile << cv::format(Xsplit[ff], cv::Formatter::FMT_CSV) << std::endl;
			pcloudFile.close();
		}
...
int test_radiallyBiasedOptimization() {
	...
	for (int k = startFrame; k <= endFrame; k++) {
		...
		for (int ff = 0; ff < 3; ff++) {
			std::ofstream pcloudFile;
			std::string filename = mainfolder + "/pointcloud/pc" + appender + std::to_string(k)
				+ "_" + std::to_string(ff) + ".csv";
			pcloudFile.open(filename.c_str());
			pcloudFile << cv::format(Xsplit[ff], cv::Formatter::FMT_CSV) << std::endl;
			pcloudFile.close();
